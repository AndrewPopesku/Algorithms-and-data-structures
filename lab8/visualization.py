...
def shellSort(draw_info):
	#logging.info("Starting sorting list...")
	lst = draw_info.lst
	gap = len(lst) // 2
	...
	while gap > 0:
		for i in range(gap, len(lst)):
			...
			lst[j] = t
			draw_list(draw_info, {j + gap: draw_info.GREEN, j: draw_info.RED}, True)
			yield True

		gap //= 2

	#logging.info("List is already sorted!")
    

def start_visualization(list_to_sort):
	#logging.info("Starting visualization of sorting list...")
	run = True
	...
	pygame.quit()
	#logging.info("Visualization has ended successfully!")
